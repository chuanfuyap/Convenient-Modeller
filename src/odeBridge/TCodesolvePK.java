...
public class TCodesolvePK {
    ...
    public TCodesolvePK(String sbmllink, String[] variables, int count, String[] names, String[] rID, String[] rID2, String[] pID){
        link=sbmllink;
    ...
    public double[][] runsolver(double[] newparam, double[] time) {
        ...
        outputarray = new double[time.length][num+proteinID.length+reactionID2.length];
        int vnamesize = 0;
        int pnamesize = 0;
        int rIDsize = 0;
        int rIDsize2 = 0;
        ...
        for(int i=0; i<proteinID.length;i++){
            proteinIDsize+=proteinID[i].length()+1;
        }
        ...
        Pointer proteinIDs = new Memory(proteinIDsize * Native.getNativeSize(Character.TYPE));
        ...
        offset = 0;
        for (int i =0; i <proteinID.length; i++) {
            proteinIDs.setString(offset, proteinID[i]);
            // null-terminator
            proteinIDs.setMemory(offset + proteinID[i].length(), 1, (byte)(0));
            offset += reactionID2[i].length() + 1;
        }
        ...
        runlib.TCrunSolverPK(link, variableNames, num, parameter, parameterIDs, parametercount, reactionIDs, reactionID2.length, reactionIDs2, timepoints, time.length-1, rowPtrRef, rowCountRef,proteinIDs, proteinID.length);        
        ...
        int counter=num+proteinID.length+reactionID2.length;
        for(int i=0;i<time.length;i++){
        ...
        runlib.TCarrayCleanupPK(rows, time.length);
